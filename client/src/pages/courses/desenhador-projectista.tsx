...
import { ArrowLeft, CheckCircle2, BookOpen, PenTool } from "lucide-react";
import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import { useCourseColor } from "@/hooks/useCourseColor";

export default function DesenhadorProjectista() {
  const { t } = useLanguage();
  useCourseColor('desenhador-projectista');
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div ref={ref} className="relative py-20 text-primary-foreground overflow-hidden">
        <div className="absolute inset-0">
          <motion.img style={{ y }} src="https://images.pexels.com/photos/5582599/pexels-photo-5582599.jpeg" alt={t('courses.dp.title')} className="absolute -top-[10%] left-0 w-full h-[120%] object-cover" />
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t('courses.dp.title')}</h1>
          <p className="text-xl md:text-2xl opacity-90 max-w-2xl">{t('courses.dp.desc')}</p>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[1, 2, 3, 4, 5, 6].map((item) => (
                  <div key={item} className="flex items-start p-4 bg-card rounded-lg border border-border shadow-sm">
                    ...
                    <span className="text-card-foreground">{t(`courses.dp.learn.item${item}`)}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <div className="bg-card p-6 rounded-xl border border-border shadow-sm sticky top-6">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between border-b border-border pb-2">
                  ...
                  <span className="font-medium">{t('courses.dp.durationValue')}</span>
                </div>
                <div className="flex justify-between border-b border-border pb-2">
                  <span className="text-muted-foreground">{t('courses.certification')}:</span>
                  <span className="font-medium">{t('courses.dp.certificationValue')}</span>
                </div>
              </div>
