...
import { ArrowLeft, CheckCircle2, BookOpen, Hammer } from "lucide-react";
import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import { useCourseColor } from "@/hooks/useCourseColor";

export default function TecnologiasMoveis() {
  const { t } = useLanguage();
  useCourseColor('tecnologias-moveis');
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div ref={ref} className="relative py-20 text-primary-foreground overflow-hidden">
        <div className="absolute inset-0">
          <motion.img style={{ y }} src="https://images.unsplash.com/photo-1600121848594-d8644e57abab?w=1920&h=600&fit=crop&q=80" alt={t('courses.ttm.title')} className="absolute -top-[10%] left-0 w-full h-[120%] object-cover" />
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t('courses.ttm.title')}</h1>
          <p className="text-xl md:text-2xl opacity-90 max-w-2xl">{t('courses.ttm.desc')}</p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-12">
            <section>
              ...
              <div className="prose prose-lg text-muted-foreground">
                <p>{t('courses.ttm.aboutText')}</p>
              </div>
            </section>

            <section>
              <h2 className="text-3xl font-bold text-foreground mb-6 flex items-center">
                <Hammer className="mr-3 h-6 w-6 text-primary" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[1, 2, 3, 4, 5, 6].map((item) => (
                  <div key={item} className="flex items-start p-4 bg-card rounded-lg border border-border shadow-sm">
                    ...
                    <span className="text-card-foreground">{t(`courses.ttm.learn.item${item}`)}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <div className="bg-card p-6 rounded-xl border border-border shadow-sm sticky top-6">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between border-b border-border pb-2">
                  ...
                  <span className="font-medium">{t('courses.ttm.durationValue')}</span>
                </div>
                <div className="flex justify-between border-b border-border pb-2">
                  <span className="text-muted-foreground">{t('courses.certification')}:</span>
                  <span className="font-medium">{t('courses.ttm.certificationValue')}</span>
                </div>
              </div>
